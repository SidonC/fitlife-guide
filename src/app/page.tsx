...
type AppState = "onboarding" | "nutrition" | "premiumChoice" | "payment" | "install" | "main";
...
if (stored) {
  ...
  if (appState === "premiumChoice") {
    return <PremiumChoice onChoosePremium={handleChoosePremium} onSkip={handleSkipPremium} />;
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      {/* Bottom Navigation - Padrão Header Perfil */}
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1 transition-all duration-300 ease-in-out transform ${
                  isActive
              ...
              >
                <Icon
                  className={`w-6 h-6 transition-all duration-300 ${
                    isActive ? "drop-shadow-[0_2px_8px_rgba(253,224,71,0.5)]" : ""
                  }`}
                  strokeWidth={isActive ? 2.5 : 2}
                />
                <span className={`text-[10px] font-semibold transition-all duration-300 ${
                  isActive ? "drop-shadow-[0_1px_4px_rgba(0,0,0,0.3)]" : ""
                }`}>
                  {tab.label}
                </span>
              </button>
